...
def test_poster_from_story_returns_poster():
    ...
    with patch(
        ...
        AsyncMock(return_value={"title": "T", "subtitle": "S", "sections": [], "footer": "F", "style": "light"}),
    ) as mock_gen:
...
@pytest.mark.asyncio
async def test_generate_poster_from_text_blocks_advice(monkeypatch):
    from app.x import poster

    async def fake_llm(_system, _user):
        return '{"title":"Buy now","subtitle":"S","sections":[{"heading":"H","bullets":["You should buy this stock now"]}],"footer":"F"}'

    monkeypatch.setattr(poster, "_llm_call", fake_llm)
    monkeypatch.setenv("DEEPSEEK_API_KEY", "test-key")

    with pytest.raises(poster.PosterError, match="blocked term"):
...
async def test_generate_poster_from_text_rejects_missing_sections(monkeypatch):
    ...
    async def fake_llm(_system, _user):
        return '{"title":"T","subtitle":"S","sections":[],"footer":"F"}'
