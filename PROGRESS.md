...
| 90 | Reddit collection runs credential-free when the REDDIT_* env is absent | reddit | Supersedes #88's "refuse to start without creds": `RedditSource` now picks praw (creds present) or rss (public `/top/.rss` Atom feed — no account/login/API key) per poll. RSS mode loses scores and the NSFW flag, so MIN_SCORE is replaced by top-of-week rank + a 200-char substance floor, and it only runs against the SFW allowlist subs. Candidate→granted rights gate unchanged — the human gate replaces the missing automated one. `REDDIT_COLLECTION_FORCE_RSS=true` pins rss mode for testing. Old "fetch requires credentials" test rewritten to assert mode selection; network layer covered by new `test_reddit_rss_fallback.py`. PM sending still requires creds. |
